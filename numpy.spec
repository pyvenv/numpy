...
# Patch for ipo
Patch0:         https://patch-diff.githubusercontent.com/raw/numpy/numpy/pull/5190.patch
...
Requires:       %{?pyvenv_name_prefix}%{name} = %{epoch}:%{version}-%{release}
Requires:       python-devel
Provides:       %{?pyvenv_name_prefix}f2py = %{version}-%{release}
Obsoletes:      %{?pyvenv_name_prefix}f2py <= 2.45.241_1927

%description f2py
This package includes a version of f2py that works properly with NumPy.

%if 0%{?with_python3}
%package -n %{?pyvenv_name_prefix}python3-numpy
Summary:        A fast multidimensional array facility for Python

Group:          Development/Languages
License:        BSD
%description -n %{?pyvenv_name_prefix}python3-numpy
NumPy is a general-purpose array-processing package designed to
efficiently manipulate large multi-dimensional arrays of arbitrary
records without sacrificing too much speed for small multi-dimensional
arrays.  NumPy is built on the Numeric code base and adds features
...
%package -n %{?pyvenv_name_prefix}python3-numpy-f2py
Summary:        f2py for numpy
Group:          Development/Libraries
Requires:       %{?pyvenv_name_prefix}python3-numpy = %{epoch}:%{version}-%{release}
...
%if 0%{?with_python3}
%files -n %{?pyvenv_name_prefix}python3-numpy
%doc LICENSE.txt README.txt THANKS.txt DEV_README.txt COMPATIBILITY site.cfg.example
...
%files -n %{?pyvenv_name_prefix}python3-numpy-f2py
%{_bindir}/f2py3
